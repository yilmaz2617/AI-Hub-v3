...
import {
  Send, Trash2, Copy, Check, Bot, User, Loader2,
  ChevronDown, Sparkles, Settings, Hash, Type
} from 'lucide-react';
...
export default function ChatPanel() {
  const {
    activeSessionId, provider, model, systemPrompt,
    setProvider, setModel, setSystemPrompt, addMessage,
    updateMessage, clearSession, getActiveSession, setIsStreaming
  } = useChatStore();
  ...
  const handleSend = useCallback(async () => {
    ...
    const userMsg: Message = { id: uid(), role: 'user', content: text, timestamp: Date.now(), provider, model };
    addMessage(sid, userMsg);

    const aiMsgId = uid();
    const aiMsg: Message = { id: aiMsgId, role: 'assistant', content: '', timestamp: Date.now(), provider, model, isStreaming: true };
    ...
    try {
      ...
      if (provider === 'pollinations') {
        ...
        if (!key) {
          throw new Error('API key bulunamadı. API Durumu panelinden ekleyin veya Pollinations kullanın.');
        }
        const history = session?.messages.filter(m => m.id !== aiMsgId).map(m => ({
          id: m.id,
  ...
  }, [input, isLoading, activeSessionId, provider, model, systemPrompt, session, apiKeys, soundEnabled, playSend, addMessage, updateMessage, setIsStreaming, streamText, addToast]);
  ...
  const displayedMessages = session?.messages.filter(msg => {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center gap-3">
          <span className="font-semibold text-sm" style={{ color: 'var(--text)' }}>Sohbet</span>
          {session && (
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            style={{ color: showSearch ? 'var(--accent)' : 'var(--text3)', background: showSearch ? 'var(--surface2)' : 'transparent' }}
            title="Ara"
          >
            <Hash size={14} />
          </button>
          ...
          <button
            ...
            style={{ background: 'var(--surface2)', border: '1px solid var(--border2)', color: 'var(--purple)' }}
      ...
      {/* Search bar */}
      {showSearch && (
        <div className="px-4 py-2 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
          <input
            type="text"
            value={searchQuery}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          {/* Provider */}
          <div>
            <label className="text-[10px] uppercase tracking-wider font-semibold block mb-1.5" style={{ color: 'var(--text3)' }}>Sağlayıcı</label>
          ...
          {/* Model */}
          <div>
            <label className="text-[10px] uppercase tracking-wider font-semibold block mb-1.5" style={{ color: 'var(--text3)' }}>Model</label>
            <div className="relative">
              <select
                value={model}
          ...
          {modelsForProvider.find(m => m.id === model)?.info && (
            <div className="text-[11px] p-2.5 rounded-lg leading-relaxed" style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text2)' }}>
              {modelsForProvider.find(m => m.id === model)?.info}
            </div>
          )}
          ...
          {/* System Prompt */}
          <div>
            <label className="text-[10px] uppercase tracking-wider font-semibold block mb-1.5" style={{ color: 'var(--text3)' }}>Sistem Prompt</label>
            <textarea
              value={systemPrompt}
          ...
          {/* Quick Prompts */}
          <div>
            <label className="text-[10px] uppercase tracking-wider font-semibold block mb-1.5" style={{ color: 'var(--text3)' }}>Hızlı</label>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {displayedMessages.map((msg) => (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: msg.role === 'assistant' ? 'rgba(88,166,255,0.15)' : 'rgba(63,185,80,0.15)',
                    border: `1px solid ${msg.role === 'assistant' ? 'rgba(88,166,255,0.3)' : 'rgba(63,185,80,0.3)'}`,
                  }}
                >
                  {msg.role === 'assistant' ? <Bot size={14} style={{ color: 'var(--accent)' }} /> : <User size={14} style={{ color: 'var(--green)' }} />}
                </div>
                ...
                >
                  ...
                  <button
                    ...
                    title="Kopyala"
                  >
                    {copiedId === msg.id ? <Check size={12} style={{ color: 'var(--green)' }} /> : <Copy size={12} />}
                  </button>
                  ...
                  {/* Meta */}
                  <div className="flex items-center gap-2 mt-2" style={{ color: 'var(--text3)', fontSize: '10px' }}>
                    <span>{new Date(msg.timestamp).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</span>
                    {msg.provider && (
                      <>
                        <span>·</span>
                        <span style={{ color: 'var(--text2)' }}>{msg.provider} · {msg.model}</span>
                      </>
                    )}
                  </div>
                </div>
          ...
          {/* Input Area */}
          <div className="flex-shrink-0 p-3" style={{ borderTop: '1px solid var(--border)', background: 'var(--surface)' }}>
            {/* Token counter */}
            {input.length > 0 && (
              <div className="flex items-center gap-3 px-1 pb-1.5 text-[10px]" style={{ color: 'var(--text3)' }}>
            ...
            <div className="flex gap-2 items-end">
              <textarea
                ...
                style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', lineHeight: '1.5' }}
              />
              {isLoading ? (
                <button
