...
import {
  Send, Crown, Trash2, Copy, Check, User, Loader2,
  ChevronDown, Timer
} from 'lucide-react';
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const reply = await callPremiumAPI(
        model, updatedHistory, systemPrompt,
      ...
      const errMsg = (e as Error).message?.includes('429')
        ...
        : (e as Error).message;
      const aiMsg: Message = { id: uid(), role: 'assistant', content: '❌ ' + errMsg, timestamp: Date.now() };
      setHistory([...updatedHistory, aiMsg]);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm" style={{ color: 'var(--text)' }}>Premium Free</span>
          <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'linear-gradient(135deg,var(--premium),var(--red))', color: '#fff' }}>
            S-Tier
          </span>
        </div>
        {queuePos > 0 && (
          <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--gold)' }}>
            ...
            <span>Sıra: {queuePos} (~{queuePos * 2}sn)</span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          {/* Badge */}
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: 'linear-gradient(135deg,rgba(255,159,67,0.15),rgba(255,107,107,0.1))', border: '1px solid var(--premium)' }}>
            <Crown size={14} style={{ color: 'var(--premium)' }} />
            <span className="text-xs font-semibold" style={{ color: 'var(--premium)' }}>PREMIUM FREE</span>
          </div>

          {/* Queue Info */}
          <div className="text-[11px] p-2.5 rounded-lg space-y-1" style={{ background: 'var(--surface3)', border: '1px solid var(--border2)', color: 'var(--text2)' }}>
            <div>⭐ <span style={{ color: 'var(--gold)', fontWeight: 600 }}>S-Tier</span> modeller</div>
            <div>⏳ Sıra: <span id="queue-pos" style={{ color: 'var(--gold)' }}>{queuePos > 0 ? queuePos : '-'}</span></div>
            <div>🕐 Tahmini: <span id="queue-eta" style={{ color: 'var(--gold)' }}>{queuePos > 0 ? queuePos * 2 + ' saniye' : '-'}</span></div>
          </div>

          {/* Model */}
          <div>
            <label className="text-[10px] uppercase tracking-wider font-semibold block mb-1.5" style={{ color: 'var(--text3)' }}>Model Seç</label>
          ...
          {/* Model Info */}
          <div className="text-[11px] p-2.5 rounded-lg" style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text2)', lineHeight: '1.6' }}>
            {PREMIUM_INFO[model] || '—'}
          </div>
          ...
          {/* Specs */}
          <div className="text-[10px] p-2.5 rounded-lg space-y-0.5" style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text2)' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ scrollbarWidth: 'thin' }}>
            {history.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full gap-3" style={{ color: 'var(--text3)' }}>
                ...
                <p className="text-xs text-center max-w-sm">En kaliteli ücretsiz modelleri kullanabilirsin. Rate limit nedeniyle bazen sıra bekleyebilirsin.</p>
              </div>
            )}
            {history.map(msg => (
              <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div
                  ...
                  style={{
                    background: msg.role === 'assistant' ? 'rgba(255,159,67,0.15)' : 'rgba(63,185,80,0.15)',
          ...
          <div className="flex-shrink-0 p-3" style={{ borderTop: '1px solid var(--border)', background: 'var(--surface)' }}>
            <div className="flex gap-2 items-end">
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg,var(--premium),var(--red))', color: '#fff' }}
              >
