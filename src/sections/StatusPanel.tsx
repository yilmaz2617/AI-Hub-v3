...
import { API_DEFS } from '@/lib/constants';
import { Wifi, WifiOff, AlertTriangle, RefreshCw, Key, Save, CheckCircle } from 'lucide-react';
...
export default function StatusPanel() {
  ...
  const [, setChecking] = useState<Set<string>>(new Set());

  useEffect(() => {
    const entries: APIKeyEntry[] = API_DEFS.map(def => ({
      id: def.id as Provider,
      name: def.name,
      key: apiKeys[def.id] || '',
      status: apiKeys[def.id] ? 'checking' : 'nokey',
      desc: def.desc,
    }));
    setApis(entries);
  }, [apiKeys]);
  ...
  const checkOne = useCallback(async (api: APIKeyEntry) => {
    ...
    try {
      const url = typeof def.testUrl === 'function' ? def.testUrl(api.key) : def.testUrl;
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 8000);
      const res = await fetch(url, {
        headers: def.testHeader(api.key),
  ...
  const statusIcons = {
    up: <Wifi size={14} style={{ color: 'var(--green)' }} />,
    down: <WifiOff size={14} style={{ color: 'var(--red)' }} />,
    checking: <RefreshCw size={14} className="animate-spin" style={{ color: 'var(--text3)' }} />,
    nokey: <AlertTriangle size={14} style={{ color: 'var(--yellow)' }} />,
    unknown: <AlertTriangle size={14} style={{ color: 'var(--text3)' }} />,
  };
  ...
  const statusBorders = {
    up: 'rgba(63,185,80,0.4)',
    down: 'rgba(247,129,102,0.4)',
    checking: 'var(--border)',
    nokey: 'rgba(227,179,65,0.4)',
    unknown: 'var(--border)',
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        <button
          onClick={checkAll}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs transition-all hover:bg-[var(--surface2)]"
          style={{ border: '1px solid var(--border2)', color: 'var(--accent)' }}
        >
          <RefreshCw size={12} /> Tümünü Kontrol Et
        </button>
      </div>
                  ...
                  <input
                    ...
                    onBlur={e => {
                      if (e.target.value.trim()) {
                        handleSave(api.id, e.target.value);
                      }
                    }}
                  />
                </div>
