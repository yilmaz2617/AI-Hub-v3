...
import {
  Send, Copy, Check, Loader2, Bot, ArrowRightLeft,
  Sliders
} from 'lucide-react';
...
export default function ComparePanel() {
    ...
    const callColumn = async (
      ...
      try {
        ...
        if (col.provider === 'pollinations') {
          ...
          if (!key) {
            setCol(c => ({ ...c, isLoading: false, response: 'API key bulunamadi. API Durumu panelinden ekleyin.' }));
            return;
          }
          reply = await callAPI(col.provider, col.model, key, newHistory, undefined, abort.signal);
        }
        setCol(c => ({ ...c, isLoading: false, response: reply, history: [...newHistory, { id: uid(), role: 'assistant', content: reply, timestamp: Date.now() }] }));
      } catch (e: unknown) {
  ...
  const renderColumn = (col: ColumnConfig, isA: boolean) => (
    <div className="flex-1 flex flex-col overflow-hidden rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      {/* Column header */}
      <div className="flex items-center justify-between px-3 py-2 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
        <div className="flex items-center gap-2">
          <Bot size={14} style={{ color: isA ? 'var(--accent)' : 'var(--purple)' }} />
          <span className="text-xs font-semibold" style={{ color: 'var(--text)' }}>
            {PROVIDERS.find(p => p.id === col.provider)?.name || col.provider}
          </span>
          <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: 'var(--surface3)', color: 'var(--text3)' }}>
        ...
        {col.response && (
          ...
          >
            {isA ? (copiedA ? <Check size={12} style={{ color: 'var(--green)' }} /> : <Copy size={12} />)
                 : (copiedB ? <Check size={12} style={{ color: 'var(--green)' }} /> : <Copy size={12} />)}
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4" style={{ scrollbarWidth: 'thin' }}>
        {col.isLoading ? (
          <div className="flex items-center gap-2 py-4">
            <Loader2 size={16} className="animate-spin" style={{ color: isA ? 'var(--accent)' : 'var(--purple)' }} />
            <span className="text-sm" style={{ color: 'var(--text3)' }}>Yaziyor...</span>
          </div>
        ) : col.response ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-2" style={{ color: 'var(--text3)' }}>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm" style={{ color: 'var(--text)' }}>Karsilastirma Modu</span>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      {/* Model settings */}
      {showSettings && (
        <div className="flex gap-4 px-4 py-3 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
          {/* Column A settings */}
          <div className="flex-1 flex gap-2">
            <select
              value={colA.provider}
              onChange={(e) => setColA(c => ({ ...c, provider: e.target.value as Provider, model: MODELS[e.target.value]?.[0]?.id || '' }))}
            ...
            <select
              ...
              style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)' }}
            >
              {modelsA.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </div>
          {/* Column B settings */}
          <div className="flex-1 flex gap-2">
            <select
              value={colB.provider}
              onChange={(e) => setColB(c => ({ ...c, provider: e.target.value as Provider, model: MODELS[e.target.value]?.[0]?.id || '' }))}
              className="flex-1 rounded-lg px-2 py-1.5 text-xs outline-none"
              style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)' }}
            >
              {PROVIDERS.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            ...
            <select
              ...
              style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)' }}
            >
              {modelsB.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </div>
        </div>
      )}
      ...
      <div className="flex-shrink-0 p-3" style={{ borderTop: '1px solid var(--border)', background: 'var(--surface)' }}>
        <div className="flex gap-2 items-center">
          ...
          >
            {(colA.isLoading || colB.isLoading) ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />}
            Gonder
          </button>
        </div>
      </div>
