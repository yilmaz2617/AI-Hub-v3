...
export const SyncSettings: React.FC = () => {
  ...
  return (
    <div className="space-y-2">
      <Toggle
        label="Otomatik Sync (2s debounce)"
        checked={settings.autoSync}
        onChange={settings.setAutoSync}
      />
      <Toggle
        label="Cross-Tab Realtime"
        checked={settings.crossTabRealtime}
        onChange={settings.setCrossTabRealtime}
      />
      <Toggle
        label="GitHub Gist Backup"
        checked={settings.githubBackup}
        onChange={settings.setGitHubBackup}
      />
      <Toggle
        label="Çakışma Modalı (Auto-resolve kapalı)"
        checked={settings.conflictModal}
        onChange={settings.setConflictModal}
      />
      <Toggle
        label="Offline Kuyruk"
        checked={settings.offlineQueue}
        onChange={settings.setOfflineQueue}
      />
      <div className="pt-4">
        <label className="block text-sm text-gray-700 dark:text-gray-300 mb-2">
          Sync Aralığı: {settings.syncInterval}ms
        </label>
        <input
          type="range"
          min="1000"
          ...
          onChange={e => settings.setSyncInterval(Number(e.target.value))}
          className="w-full"
        />
      </div>
    </div>
