...
export const useUIStore = create<UIState>((set) => ({
  ...
  showOnboarding: (() => {
    try { return !localStorage.getItem('aihub_onboarding_done'); } catch { return true; }
  })(),
  ...
  setOnboarding: (v) => {
    ...
    if (!v) try { localStorage.setItem('aihub_onboarding_done', '1'); } catch { /* */ }
  },
  setCompareMode: (v) => set({ showCompareMode: v }),
  setActiveRightPanel: (panel) => set(s => ({ activeRightPanel: s.activeRightPanel === panel ? null : panel })),
